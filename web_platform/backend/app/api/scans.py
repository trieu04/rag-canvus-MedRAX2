    ...
    try:
        for file in files:
            ...
            try:
                ...
                # Get file size from file object or default to 0 if not available
                file_size = file.size or 0
            except ValueError as e:
                logger.warning(f"Upload rejected during save: {e}")
                raise HTTPException(
                    status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="File exceeds maximum allowed size"
                )
